<script>
	import { Avatar, Button, Icon, Input, PageBody, PageHeader, PageWrapper } from '@svind/svelte';
	...
	async function loadMessages() {
		const result = await fetch('/api/messages').then((res) => res.json());
		console.log(result);
	...
	async function onSend() {
		console.log('send');
		console.log('other: ', activeChat.id);
		const data = {
			text: newMessageText,
...
<PageWrapper>
	...
	<div class="-mx-4 flex flex-col overflow-auto p-1 w-screen h-full bg-yellow-100">
		{#each messages as message}
			<Message
				createdAt={message.createdAt}
				text={message.content}
				other={message.sender === activeChat.id}
			/>
		{/each}
	</div>
	<div class="flex gap-2 -mx-4 p-2 bg-yellow-100">
		...
		/>
		<button class="btn btn-success text-2xl bg-green-600 btn-circle" on:click={onSend}>
